...
def get_questions_db(update,context):
    callback_query_response = json.loads(update.callback_query.data)
    exam_paper_id=callback_query_response['exam_paper_id']
    print(" I am from the get questions from db ")
    print(exam_paper_id)
    print( " I am nikihl ")
    query ='''{
...
def send_quiz(update,context):
  ...
  global correct_ans
  global wrong_ans
  global is_playing_quiz

  chat_id=get_chat_id(update,context)
  bot_id=context.bot.id
  bot=context.bot
  print(" This is the all quiz questions from the send quiz ")
  print(all_quiz_questions)

  if static_counter.counter == -1 :
    static_counter()
    all_quiz_questions = get_questions_db(update,context)
    is_playing_quiz=True
    ...
    bot.send_message(chat_id,text=score)
    is_playing_quiz=False 
    tem={"chat_id":chat_id,"questions":user_result,"bot_id":bot_id,"exam_paper_id":6641}
    store_result(tem)

# Store the user quiz result in the Database 
def store_result(tem):
  pass
